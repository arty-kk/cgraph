...
def test_async_database_url_normalization(monkeypatch) -> None:
    ...
    monkeypatch.setattr(app_config.settings, "database_url", "postgresql://user:pass@localhost:5432/dbname")
...
def test_async_engine_uses_pool_settings_from_config(monkeypatch) -> None:
    ...
    with monkeypatch.context() as patch_ctx:
        patch_ctx.setattr(
            app_config.settings,
            "database_url",
            "postgresql://pool-user:pool-pass@localhost:5432/pooldb",
        )
        patch_ctx.setattr(app_config.settings, "db_pool_size", 7)
        patch_ctx.setattr(app_config.settings, "db_max_overflow", 3)
        patch_ctx.setattr(app_config.settings, "db_pool_timeout_seconds", 12.5)
        patch_ctx.setattr(app_config.settings, "db_pool_recycle_seconds", 321.0)
