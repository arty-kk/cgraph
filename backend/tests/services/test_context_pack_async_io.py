...
@pytest.mark.anyio
async def test_pack_context_concurrent_requests_use_cache_offload_path(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    session = _Session()
    client = _CacheClient()
    ...
    async def _fake_run_cpu_io_async(fn, *args, operation=None, **kwargs):
        _ = kwargs
        operations.append(operation or fn.__name__)
        return fn(*args)

    async def _read_file_async(path: Path, max_chars: int) -> str:
        await asyncio.sleep(0.002)
        return (f"{path.name}:payload")[:max_chars]
    ...
    monkeypatch.setattr(cache_module, "get_async_redis_client", lambda: client)
    ...
    assert elapsed < 4
    assert "cache.serialize_set" in operations
    assert "cache.deserialize_get" in operations
