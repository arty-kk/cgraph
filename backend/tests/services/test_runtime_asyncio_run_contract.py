...
def test_runtime_modules_do_not_use_asyncio_run_outside_allowlist() -> None:
    ...
    assert not violations, "asyncio.run is allowed only in celery worker adapter: " + ", ".join(violations)
...
def test_runtime_modules_do_not_use_run_coroutine_threadsafe() -> None:
    ...
    for path in sorted(APP_ROOT.rglob("*.py")):
        ...
        if _contains_forbidden_calls(path):
            violations.append(rel)
    assert not violations, "run_coroutine_threadsafe is forbidden in runtime modules: " + ", ".join(violations)
