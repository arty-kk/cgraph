...
## Сквозные принципы масштабирования

- **Статус‑less API**: все состояния задач/артефактов/графов сохраняются в БД/объектном хранилище, API можно горизонтально масштабировать без sticky‑sessions.
- **Разделение compute**: тяжёлые задачи уходят в очереди `heavy/medium/light`, можно независимо масштабировать воркеры и API.
- **Горячий контур**: кеширование и лимиты (Redis) защищают Postgres и LLM‑квоты при пиковых нагрузках.
- **Предсказуемое хранилище**: все большие артефакты (патчи, snapshots, отчёты) — в object storage с TTL/signed URLs.
- **Наблюдаемость и SLO**: метрики latency/queue time/ошибок/стоимости LLM, алерты и автоматическое масштабирование.
- **Горизонтальное масштабирование UI**: фронтенд — статические ассеты через CDN, API‑endpoint за балансировщиком.
